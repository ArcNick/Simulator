...
def visualize_vp(vp, nz=706, nx=690, dx=1.5, dz=1.5, cmap='jet', save_path=None):
    ...
    # 计算实际物理范围 [xmin, xmax, zmax, zmin]
    # 注意：这里 zmin 在后，zmax 在前，配合 imshow 默认行为实现深度向下增加
    extent = [0, nx, nz, 0]
    ...
    # 设置坐标轴标签
    plt.title(f'Vp Model Visualization', fontsize=14, pad=20)
    plt.xlabel('Horizontal Distance (m)', fontsize=12)
    plt.ylabel('Depth (m)', fontsize=12)
    
    # 将 X 轴刻度置于顶部（符合地学软件习惯）
...
vp.tofile(os.path.join(".", "Vp_brother.bin"))
visualize_vp(vp)
